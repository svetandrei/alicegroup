@if($menus)
        ...
        <table class="table table-hover">
            ...
            @foreach($menus as $menu)
                <tr>
                    ...
                    <td class="align-left">{!! link_to_route('menu.edit', $menu->title, ['menu' => $menu->id]) !!}</td>
                    <td class="align-left">
                        @if ($menu->publish > 0)
                    ...
                    <td>{{$menu->url}}</td>
                    <td>
                        {!! Form::open(['url' => route('menu.destroy', ['menu' => $menu->id]), 'class'=>'form-horizontal','method'=>'POST']) !!}
                        @method('DELETE')
                        @csrf
    ...
    {!! link_to_route('menu.create', 'Добавить пункт', null,['class' => 'btn btn-primary']) !!}
