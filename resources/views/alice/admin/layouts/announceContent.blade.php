@if($announces)
    ...
    <div class="table-responsive my-4">
        <table class="table table-hover">
            ...
            @foreach($announces as $announce)
                <tr>
                    <td class="align-left">{{$announce->id}}</td>
                    <td class="align-left">{!! link_to_route('announce.edit', $announce->title, ['announce' => $announce->alias]) !!}</td>
                    <td class="align-left">
                    ...
                    <td>{{(isset($announce->author['title']))?$announce->author['title']:'-'}}</td>
                    <td>
                        {!! Form::open(['url' => route('announce.destroy', ['announce' => $announce->alias]), 'class'=>'form-horizontal','method'=>'POST']) !!}
                        @method('DELETE')
                        @csrf
    ...
    {!! link_to_route('announce.create', 'Добавить материал', null,['class' => 'btn btn-primary']) !!}
@endif