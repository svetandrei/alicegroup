@if(isset($accordions))
    ...
    <div class="table-responsive my-4">
        <table class="table table-hover">
            ...
            @foreach($accordions as $accordion)
                <tr>
                    <td class="align-left">{{$accordion->id}}</td>
                    <td class="align-left">{!! link_to_route('accordion.edit', $accordion->title, ['accordion' => $accordion->id]) !!}</td>
                    <td class="align-left">
                        @if ($accordion->publish > 0)
                    ...
                    <td>{{(isset($accordion->service['title']))?$accordion->service['title']:'Услуги'}}</td>
                    <td>
                        {!! Form::open(['url' => route('accordion.destroy', ['accordion' => $accordion->id]), 'class'=>'form-horizontal','method'=>'POST']) !!}
                        @method('DELETE')
                        @csrf
    ...
    {!! link_to_route('accordion.create', 'Добавить материал', null,['class' => 'btn btn-primary']) !!}
@endif