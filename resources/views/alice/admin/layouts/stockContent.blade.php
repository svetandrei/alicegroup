@if($stocks)
    ...
    <div class="table-responsive my-4">
        <table class="table table-hover">
            ...
            @foreach($stocks as $stock)
                <tr>
                    <td class="align-left">{{$stock->id}}</td>
                    <td class="align-left">{!! link_to_route('stock.edit', $stock->title, ['stock' => $stock->alias]) !!}</td>
                    <td class="align-left">
                        @if ($stock->publish > 0)
                    ...
                    <td>
                        {!! Form::open(['url' => route('stock.destroy', ['stock' => $stock->alias]), 'class'=>'form-horizontal','method'=>'POST']) !!}
                        @method('DELETE')
                        @csrf
    ...
    {!! link_to_route('stock.create', 'Добавить материал', null,['class' => 'btn btn-primary']) !!}
@endif