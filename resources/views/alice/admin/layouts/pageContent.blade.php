@if($pages)
    ...
    <div class="table-responsive my-4">
        <table class="table table-hover">
            ...
            @foreach($pages as $page)
                <tr>
                    <td class="align-left">{{$page->id}}</td>
                    <td class="align-left">{!! link_to_route('page.edit', $page->title, ['page' => $page->alias]) !!}</td>
                    <td class="align-left">
                        @if ($page->publish > 0)
                    ...
                    <td>{{$page->alias}}</td>
                    <td>
                        {!! Form::open(['url' => route('page.destroy', ['page' => $page->alias]), 'class'=>'form-horizontal','method'=>'POST']) !!}
                        @method('DELETE')
                        @csrf
    ...
    {!! link_to_route('page.create', 'Добавить страницу', null,['class' => 'btn btn-primary']) !!}
@endif