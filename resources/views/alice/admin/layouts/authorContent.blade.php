@if(isset($authors))
    ...
    <div class="table-responsive my-4">
        <table class="table table-hover">
            ...
            @foreach($authors as $author)
                <tr>
                    <td class="align-left">{{$author->id}}</td>
                    <td class="align-left">{!! link_to_route('author.edit', $author->title, ['author' => $author->id]) !!}</td>
                    <td class="align-left">
                        @if ($author->publish > 0)
                    ...
                    <td>{{(isset($author->announcement['title']))?$author->announcement['title']:'-'}}</td>
                    <td>
                        {!! Form::open(['url' => route('author.destroy', ['author' => $author->id]), 'class'=>'form-horizontal','method'=>'POST']) !!}
                        @method('DELETE')
                        @csrf
    ...
    {!! link_to_route('author.create', 'Добавить материал', null,['class' => 'btn btn-primary']) !!}
@endif